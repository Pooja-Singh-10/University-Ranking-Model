...
public class TotalOccupancyDashboard extends JPanel {
            ...
            for (final String semester : dg.courseSchedules) {
                if (semester.contains("2020")) {
                    ...
                    for (final String course : department.mostPopularCourse(semester).keySet()) {
                        seatsOccupied += department.mostPopularCourse(semester).get(course);
                    }
                    dataset.addValue(seatsOccupied, departmentKey, semester);
                }
            }
        }
        return dataset; 
